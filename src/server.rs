...
struct RaInstance {
    message_readers: Arc<RwLock<HashMap<u16, mpsc::Sender<Message>>>>,
...
impl RaInstance {
    fn spawn(project_root: &Path) -> Result<RaInstance> {
        ...
        let message_readers = Arc::new(RwLock::new(HashMap::new()));
        let message_readers2 = Arc::clone(&message_readers);
        task::spawn(async move {
            if let Err(err) = read_server_socket(stdout, message_readers2).await {
...
async fn process_client(socket: TcpStream, port: u16, registry: Arc<Mutex<HashMap<PathBuf, RaInstance>>>) -> Result<()> {
    ...
    ensure!(proto_init.args.is_empty(), "unexpected args passed to client");
    ...
    let instance = match registry.entry(project_root.clone()) {
        ...
        RaInstance::spawn(&project_root).context("spawn ra instance")?
        ),
    };
    ...
    message_readers.insert(port, tx);
...
async fn read_server_socket<R>(
    mut reader: R,
    senders: Arc<RwLock<HashMap<u16, mpsc::Sender<Message>>>>,
) -> Result<()>
where
    R: AsyncBufRead + Unpin,
{
